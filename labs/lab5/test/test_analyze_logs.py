...
class TestParseEntry:
    ...
    def test_convert_to_namedtuple(self, entry, expected):
        tuple_entry = analyze_ssh_logs.parse_entry(entry)
        assert tuple_entry.month_day == expected['month_day']
        assert tuple_entry.time == expected['time']
...
class TestGetIPv4Logs:
    ...
    def test_correct_cases(self, entry, expected_addresses):
        actual_addresses = analyze_ssh_logs.get_ipv4s_from_log(analyze_ssh_logs.parse_entry(entry))
    ...
    def test_cases_without_addresses(self, entry):
        assert not analyze_ssh_logs.get_ipv4s_from_log(analyze_ssh_logs.parse_entry(entry))
...
class TestGetUserFromLog:
    ...
    def test_cases_with_users(self, entry, expected_user):
        actual_user = analyze_ssh_logs.get_user_from_log(analyze_ssh_logs.parse_entry(entry))
    ...
    def test_cases_without_users(self, entry):
        assert not analyze_ssh_logs.get_user_from_log(analyze_ssh_logs.parse_entry(entry))
